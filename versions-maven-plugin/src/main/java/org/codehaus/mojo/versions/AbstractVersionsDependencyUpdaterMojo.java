...
public abstract class AbstractVersionsDependencyUpdaterMojo extends AbstractVersionsUpdaterMojo {
    ...
    protected boolean updateDependencyVersion(
            MavenProject project, MutableXMLStreamReader pom, DependencyVersionChange change)
            throws XMLStreamException {
        boolean updated = false;
        if (getProcessParent() && project.getParent() != null) {
        ...
        if (PomHelper.setDependencyVersion(
            ...
            if (getLog().isInfoEnabled()) {
                getLog().info(String.format(
                        "Updated %s:%s:%s to version %s in %s:%s:%s",
                        change.getGroupId(),
                        change.getArtifactId(),
                        change.getOldVersion(),
                        change.getNewVersion(),
