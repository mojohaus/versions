...
public class PluginUpdatesReportTest
{
    private static class TestPluginUpdatesReport extends PluginUpdatesReport
    {
        ...
        public TestPluginUpdatesReport( List<Plugin> plugins, List<Plugin> pluginManagement )
        {
            mockPlexusComponents();

            project = new MavenProject();
            project.setBuild( new Build() );
            project.getBuild().setPlugins( plugins );
            project.getBuild().setPluginManagement( new PluginManagement() );
            project.getBuild().getPluginManagement().setPlugins( pluginManagement );
    ...
    public void testOnlyUpgradablePlugins() throws IOException, MavenReportException
    {
        ...
        new TestPluginUpdatesReport( Arrays.asList( pluginOf( "artifactA" ), pluginOf( "artifactB" ),
                pluginOf( "artifactC" ) ), EMPTY_LIST )
        {
            {
                onlyUpgradable = true;
            }
    ...
    public void testOnlyUpgradableWithPluginManagement() throws IOException, MavenReportException
    {
        OutputStream os = new ByteArrayOutputStream();
        SinkFactory sinkFactory = new Xhtml5SinkFactory();
        new TestPluginUpdatesReport( EMPTY_LIST,
                Arrays.asList( pluginOf( "artifactA" ), pluginOf( "artifactB" ),
                        pluginOf( "artifactC" ) ) )
        {
            {
                onlyUpgradable = true;
    ...
    public void testOnlyProjectPlugins() throws IOException, MavenReportException
    {
        OutputStream os = new ByteArrayOutputStream();
        SinkFactory sinkFactory = new Xhtml5SinkFactory();
        new TestPluginUpdatesReport( Collections.singletonList( pluginOf( "artifactA" ) ),
                Arrays.asList( pluginOf( "artifactA" ), pluginOf( "artifactB" ),
                        pluginOf( "artifactC" ) ) )
        {
            {
                onlyUpgradable = true;
                onlyProjectPlugins = true;
            }
        }.generate( sinkFactory.createSink( os ), sinkFactory, Locale.getDefault() );
