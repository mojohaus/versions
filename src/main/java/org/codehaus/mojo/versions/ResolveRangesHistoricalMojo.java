 ...
 * @since 1.0-alpha-3
 */
@Mojo( name = "resolve-ranges-historical", requiresProject = true, requiresDirectInvocation = true, threadSafe = true )
public class ResolveRangesHistoricalMojo
    extends AbstractVersionsDependencyUpdaterMojo
{
    @Parameter( property = "versionsAsOf" )
    private String rawVersionsAsOf = null;
        ...
        for ( Dependency dep : dependencies )
        {
            ...
            if ( versionMatcher.find() )
            {
                ...
                if ( artifact != null && isIncluded( artifact ) )
                {
                    ...
                    {
                        ArtifactVersion latestVersion =
                            findLatestVersionHistorical( artifact, artifact.getVersionRange(), allowSnapshots, false, getParsedVersionsAsOf() );
    ...
    private ZonedDateTime getParsedVersionsAsOf() {
        // TODO: add error handling if the value can't be parsed
        if (this.rawVersionsAsOf != null) {
            return ZonedDateTime.parse(this.rawVersionsAsOf);
        }
