...
public class CompareDependenciesMojo extends AbstractVersionsDependencyUpdaterMojo {
    ...
    protected void update(MutableXMLStreamReader pom)
        ...
        if (getProject().getDependencies() != null && getProcessDependencies()) {
            versionChanges.addAll(compareVersions(
                    getProject(), pom, getProject().getDependencies(), remoteDepsMap, DEPENDENCY_UPDATE));
        }
        ...
        if (getProject().getParent() != null && remoteMavenProject.getParent() != null && getProcessParent()) {
            ...
            versionChanges.addAll(compareVersions(
                    getProject(), pom, singletonList(getParentDependency()), remoteDepsMap, PARENT_UPDATE));
        }
        ...
        for (Dependency dep : dependencies) {
            ...
            if (remoteDep != null) {
                ...
                if (VersionStringComparator.STRICT.compare(remoteVersion, dep.getVersion()) != 0) {
                    ...
                    if (!reportMode) {
                        updateDependencyVersion(project, pom, versionChange);
                    }
                }
            }
        }
