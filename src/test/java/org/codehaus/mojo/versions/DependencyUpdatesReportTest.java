...
public class DependencyUpdatesReportTest
{
    private static class TestDependencyUpdatesReport extends DependencyUpdatesReport
    {
        ...
        public TestDependencyUpdatesReport( List<Dependency> dependencies,
                                            List<Dependency> originalDependencyManagement,
                                            List<Dependency> dependencyManagement )
        {
            ...
            project.getOriginalModel().getDependencyManagement().setDependencies( originalDependencyManagement );

            project.getModel().setDependencyManagement( new DependencyManagement() );
            project.getModel().getDependencyManagement().setDependencies( dependencyManagement );
    ...
    private Dependency dependencyOf( String artifactId )
    {
    ...
    public void testOnlyUpgradableDependencies() throws IOException, MavenReportException
    {
        ...
        new TestDependencyUpdatesReport(
                Arrays.asList( dependencyOf( "artifactA" ), dependencyOf( "artifactB" ),
                        dependencyOf( "artifactC" ) ), EMPTY_LIST, EMPTY_LIST )
        {
            {
                onlyUpgradable = true;
            }
    ...
    public void testOnlyUpgradableWithOriginalDependencyManagement() throws IOException, MavenReportException
    {
        OutputStream os = new ByteArrayOutputStream();
        SinkFactory sinkFactory = new Xhtml5SinkFactory();
        new TestDependencyUpdatesReport( EMPTY_LIST,
                Arrays.asList( dependencyOf( "artifactA" ), dependencyOf( "artifactB" ),
                        dependencyOf( "artifactC" ) ), EMPTY_LIST )
        {
            {
                processDependencyManagement = true;
                onlyUpgradable = true;
            }
        }.generate( sinkFactory.createSink( os ), sinkFactory, Locale.getDefault() );
    ...
    public void testOnlyUpgradableWithTransitiveDependencyManagement() throws IOException, MavenReportException
    {
        OutputStream os = new ByteArrayOutputStream();
        SinkFactory sinkFactory = new Xhtml5SinkFactory();
        new TestDependencyUpdatesReport( EMPTY_LIST, EMPTY_LIST,
                Arrays.asList( dependencyOf( "artifactA" ), dependencyOf( "artifactB" ),
                        dependencyOf( "artifactC" ) ) )
        {
            {
                processDependencyManagement = true;
                processDependencyManagementTransitive = true;
                onlyUpgradable = true;
            }
        }.generate( sinkFactory.createSink( os ), sinkFactory, Locale.getDefault() );
    ...
    public void testOnlyProjectDependencies() throws IOException, MavenReportException
    {
        OutputStream os = new ByteArrayOutputStream();
        SinkFactory sinkFactory = new Xhtml5SinkFactory();
        new TestDependencyUpdatesReport( Collections.singletonList( dependencyOf( "artifactA" ) ),
                Arrays.asList( dependencyOf( "artifactA" ), dependencyOf( "artifactB" ),
                        dependencyOf( "artifactC" ) ), EMPTY_LIST )
        {
            {
                processDependencyManagement = true;
                onlyProjectDependencies = true;
            }
        }.generate( sinkFactory.createSink( os ), sinkFactory, Locale.getDefault() );
